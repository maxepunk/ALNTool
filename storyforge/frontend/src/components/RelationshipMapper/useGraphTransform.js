...
export default function useGraphTransform({
  ...
  const { nodes, edges, error } = useMemo(() => {
    // logger.debug('[useGraphTransform DEBUG] ALL parentId assignments ENABLED (Dagre rank error testing - default state).');
    // ADDED FOR DEBUGGING: General layout calculation log
    // logger.debug(`[useGraphTransform DEBUG] Recalculating layout. Entity: ${entityName} (ID: ${entityId}), Depth: ${depth}, NodeFilters: ${JSON.stringify(nodeFilters)}, EdgeFilters: ${JSON.stringify(edgeFilters)}, ActFocus: ${actFocusFilter}, Themes: ${JSON.stringify(themeFilters)}, MemorySet: ${memorySetFilter}`);
    // logger.debug('[useGraphTransform DEBUG] layoutOptions received:', JSON.stringify(layoutOptions));


    // logger.debug('[useGraphTransform] Memo Start. Entity:', entityName, 'ID:', entityId, 'Depth:', depth); // Original log
    // logger.debug('[useGraphTransform] Input graphData nodes:', graphData?.nodes?.length, 'edges:', graphData?.edges?.length); // Original log
    ...
    try {
      ...
      // logger.debug('[useGraphTransform] Step 1 (transformToGraphElements) Done. Base nodes:', baseNodes?.length, 'Base edges:', baseEdges?.length); // Original log
      ...
      // logger.debug('[useGraphTransform] Step 2 (parentId assignment) Done.'); // Original log
      ...
      // logger.debug('[useGraphTransform] Step 2.5 (isActualParentGroup) Done.'); // Original log
      ...
      // logger.debug('[useGraphTransform] Step 3 (filterGraph) Done. Filtered nodes:', filteredNodes?.length, 'Filtered edges:', filteredEdges?.length); // Original log
      ...
      // logger.debug(`[useGraphTransform DEBUG] BEFORE getDagreLayout. Is Fullscreen: ${isFullScreenForLogging}. finalNodesForLayout count: ${finalNodesForLayout?.length}.`);
      if (finalNodesForLayout && finalNodesForLayout.length > 0) {
        ...
        // logger.debug(`[useGraphTransform DEBUG] Sample node for Dagre (ID: ${sampleNode.id}, Name: ${sampleNode.data?.label}, parentId: ${sampleNode.data?.parentId})`);
      }
      // logger.debug(`[useGraphTransform DEBUG] filteredEdges count: ${filteredEdges?.length}. finalLayoutOptions for Dagre:`, JSON.stringify(finalLayoutOptions));
      // logger.debug('[useGraphTransform] Step 4: About to call getDagreLayout.'); // Original Log
      // logger.debug('[useGraphTransform] Nodes for Dagre:', finalNodesForLayout?.length, 'Edges for Dagre:', filteredEdges?.length); // Original Log
      // if (finalNodesForLayout && finalNodesForLayout.length > 0) { // Original Log
      //    logger.debug('[useGraphTransform] Sample node for Dagre:', JSON.stringify(finalNodesForLayout[0]));
      // }
      // logger.debug('[useGraphTransform] Layout options for Dagre:', JSON.stringify(finalLayoutOptions)); // Original Log
      ...
      // logger.debug(`[useGraphTransform DEBUG] AFTER getDagreLayout. Is Fullscreen: ${isFullScreenForLogging}. layoutedNodes count: ${layoutedNodes?.length}.`);
      if (layoutedNodes && layoutedNodes.length > 0) {
        const sampleLayoutedNode = layoutedNodes.find(n => n.id === entityId) || layoutedNodes[0];
        // logger.debug(`[useGraphTransform DEBUG] Sample layouted node (ID: ${sampleLayoutedNode.id}, Name: ${sampleLayoutedNode.data?.label}) position (x,y):`, sampleLayoutedNode.position?.x, sampleLayoutedNode.position?.y);
      }
      // logger.debug('[useGraphTransform] getDagreLayout call completed. Layouted nodes:', layoutedNodes?.length); // Original log
      ...
      // logger.debug('[useGraphTransform] parentNode mapping complete. Final nodes:', finalReactFlowNodes?.length); // Original log

      return { nodes: finalReactFlowNodes, edges: layoutedEdges, error: null };
    } catch (err) {
      // logger.error('[useGraphTransform] CRITICAL ERROR in memoized calculation:', err);
