...
/**
 * NOTION PROPERTY MAPPER - Data Flow & Field Types
 * 
 * This mapper handles the first phase of the sync pipeline: mapping Notion data to SQLite.
 * It transforms Notion API objects into our internal database schema.
 * 
 * DATA FLOW PHASES:
 * 1. NOTION SYNC: This file maps Notion properties → SQLite schema (SOURCE FIELDS)
 * 2. RELATIONSHIP SYNC: Populates join tables and relationship data
 * 3. COMPUTE PHASE: ComputeOrchestrator runs computers to add computed fields
 * 4. CACHE PHASE: Optional caching for performance
 * 
 * FIELD TYPES:
 * 
 * SOURCE FIELDS (mapped by this file):
 * - Come directly from Notion databases
 * - Mapped using extract* helper functions
 * - Always available after entity sync phase
 * 
 * COMPUTED FIELDS (added during compute phase):
 * - Generated by Computer classes (ActFocusComputer, ResolutionPathComputer, etc.)
 * - Only available after ComputeOrchestrator runs
 * - May be null if computation fails or dependencies missing
 * 
 * PARSED FIELDS (hybrid):
 * - Extracted from source fields using parsing logic (e.g., SF_RFID from description)
 * - Available immediately after entity sync
 * 
 * COMPUTED FIELD INVENTORY:
 * 
 * Timeline Events:
 * - act_focus: Computed from related elements' acts
 * 
 * Characters:
 * - resolution_paths: Computed from owned elements/puzzles
 * - total_memory_value: Computed from owned elements
 * - linkedCharacters: Populated from character_links join table
 * 
 * Puzzles:
 * - resolution_paths: Computed from relationships and requirements
 * - computed_narrative_threads: Computed from related elements
 * 
 * Elements:
 * - resolution_paths: Computed from container/owner relationships
 * - Memory fields: parsed_sf_rfid, sf_value_rating, sf_memory_type, sf_group, sf_group_multiplier
 * 
 * API EXPECTATIONS:
 * - Source fields: Always present in API responses
 * - Computed fields: May be null if compute phase hasn't run or failed
 * - Frontend should handle null computed fields gracefully
 */
...
async function mapCharacterWithNames(notionCharacter, notionService) {
  try {
    ...
    const mappedChar = {
      ...
      // NOTE: Characters receive computed fields during the ComputeOrchestrator phase:
      // - resolution_paths: Computed by ResolutionPathComputer from owned elements/puzzles
      // - total_memory_value: Computed by MemoryValueComputer from owned elements
      // - linkedCharacters: Populated from character_links join table after relationship sync
      linkedCharacters: [] // Populated during relationship sync phase
    };
    return mappedChar;
  } catch (error) {
...
async function mapTimelineEventWithNames(notionEvent, notionService) {
  try {
    ...
    const mappedEvent = {
      ...
      // NOTE: Timeline events receive computed fields during the ComputeOrchestrator phase:
      // - act_focus: Computed by ActFocusComputer from related elements' acts
      // These computed fields are added to SQLite after sync, not mapped from Notion here.
    };
    return mappedEvent;
  } catch (error) {
...
async function mapPuzzleWithNames(notionPuzzle, notionService) {
  try {
    ...
    const mappedPuzzle = {
      ...
      // NOTE: Puzzles receive computed fields during the ComputeOrchestrator phase:
      // - resolution_paths: Computed by ResolutionPathComputer from relationships and requirements
...
async function mapElementWithNames(notionElement, notionService) {
  try {
    ...
    const mappedElement = {
      ...
      // NOTE: Elements receive computed fields during the ComputeOrchestrator phase:
      // - resolution_paths: Computed by ResolutionPathComputer from container/owner relationships
      // Memory-related fields (parsed_sf_rfid, sf_value_rating, etc.) are parsed from description text during sync
      // These computed fields are added to SQLite after sync, not mapped from Notion here.
    };
