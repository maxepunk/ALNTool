...
function connect(dbPathOverride) {
  ...
  if (!pathToUse) {
    ...
    logger.debug(`Connecting to database at: ${pathToUse}`);
...
function getDB() {
  if (!db) {
    ...
    logger.warn('getDB() called before initializeDatabase or outside of test setup. Connecting to default production DB.');
    connect();
  }
  return db;
}
...
function initializeDatabase(dbPathOverride = null) {
  if (db) {
    logger.debug('Database already initialized. Closing existing connection before re-initializing.');
    db.close();
    db = null;
  }
  ...
  logger.debug(`Initializing database with path: ${effectivePath}`);
  ...
  try {
    ...
    logger.debug('Database initialization and migrations complete.');
  } catch (error) {
